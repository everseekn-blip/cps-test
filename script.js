...
function updateDisplay() {
    ...
    if (state.isRunning && state.timeLimit > 0) {
        ...
        timerText.textContent = `${remaining.toFixed(1)}s kaldı`;
...
function startGame() {
    ...
    if (state.timeLimit > 0) {
        ...
        timerText.textContent = '∞ mod - tıklamaya devam!';
    }
...
function endGame() {
    ...
    // Show result
    if (state.currentCps >= state.maxCps) {
        timerText.textContent = `🏆 YENİ REKOR: ${state.currentCps} CPS! 🏆`;
    }
    ...
    // Auto-submit score if player has a name
    const name = playerNameInput.value.trim();
    if (name && state.currentCps > 0) {
        autoSubmitScore(name, state.currentCps, state.timeLimit || 0);
    }
}

async function autoSubmitScore(name, cps, mode) {
    const success = await submitScore(name, cps, mode);
    if (success) {
        // Calculate rank
        ...
        const rank = filtered.findIndex(e => e.name.toLowerCase() === name.toLowerCase() && e.cps === cps) + 1;
        if (rank > 0 && rank <= 20) {
            showRankNotification(rank, mode);
        }
    }
}

function showRankNotification(rank, mode) {
    const notif = document.createElement('div');
    notif.id = 'rank-notification';
    const rankText = rank === 1 ? '🥇 1.' : rank === 2 ? '🥈 2.' : rank === 3 ? '🥉 3.' : `${rank}.`;
    notif.innerHTML = `<span class="rank-notif-text">Sıralamada ${rankText} sıradasın! (${mode}s)</span>`;
...
function resetGame() {
    ...
    clickCount.textContent = '0 tıklama';
    timerBar.style.width = '100%';
...
async function submitScore(name, cps, mode) {
    ...
    if (!isJsonBinConfigured()) {
        // Local mode - keep best per player per mode
        const data = getLocalLeaderboard();
        upsertScore(data, entry);
...
// Keep only the best score per player per mode (update if new score is higher)
function upsertScore(data, entry) {
    const idx = data.findIndex(e =>
        e.name.toLowerCase() === entry.name.toLowerCase() &&
        e.mode == entry.mode
    );
    if (idx >= 0) {
        if (entry.cps > data[idx].cps) {
            data[idx].cps = entry.cps;
            data[idx].date = entry.date;
        }
    } else {
        data.push(entry);
    }
